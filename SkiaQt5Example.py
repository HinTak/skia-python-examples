...
# -*- coding: utf-8 -*-
#
#  Skia + PyQt5 (QOpenGLWidget) example
#
#  Copyright 2024 Hin-Tak Leung (adapted to PyQt5 by Copilot)
#  Distributed under the terms of the new BSD license.
#
#  Based on SkiaSDLExample.py and Google's Skia + SDL C++ example.
#
#  This port replaces SDL2 with PyQt5, preserving Skia and OpenGL API calls,
...
class SkiaQOpenGLWidget(QOpenGLWidget):
    ...
    def mousePressEvent(self, event):
        if event.button() == QtCore.Qt.MouseButton.LeftButton:
            rect = Rect.MakeLTRB(event.x(), event.y(), event.x(), event.y())
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Skia + PyQt5 Example")
